    ...
    def _observe_perspective(self,
        ...
        choice_prompt = ChatPromptTemplate.from_messages(
            [
                ...
                ("user", "{surrounding_prompt}" if "retrieved_information" in kwargs.keys() else ''),   # TODO optimize prompt
                ("user", "{history_nodes_prompt}" if "history_nodes" in kwargs.keys() else ''),  # TODO
                ("user", "{query}"),
                ("user", str_content),
                ("user", " ##Output:")
            ]
        ...
        if "retrieved_information" in kwargs.keys():
            params.update({
                "surrounding_prompt": kwargs["retrieved_information"]
            })
        if "history_nodes_prompt" in kwargs.keys():
            params.update({
                "history_nodes_prompt": kwargs["history_nodes_prompt"], 
            })
