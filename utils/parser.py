...
class CityWalkerParser(PydanticOutputParser):
    ...
    def parse_result(
        ...
        MapLogger.debug(f"Parsing result...")

        json_str = result[0].text
        MapLogger.debug(f"Original input before any fixing: {json_str}")

        try:
            MapLogger.debug("Trying to directly parse the output...")
            parsed_object = self.pydantic_object.parse_obj(json.loads(json_str))
            MapLogger.debug("Successfully parsed JSON directly!")

            return parsed_object
        except (ValidationError, json.JSONDecodeError) as e:
            MapLogger.debug(f"Direct parsing failed: {e}")

        try:
            MapLogger.debug("Attempting to fix JSON format...")
            corrected_json = self._fix_json_format(json_str)
            MapLogger.debug(f"Corrected JSON: {corrected_json}")

            parsed_object = self.pydantic_object.parse_obj(json.loads(corrected_json))

            MapLogger.debug("Successfully parsed JSON after fixing format!")
            return parsed_object

        except (ValidationError, json.JSONDecodeError) as e:
            MapLogger.debug(f"Parsing after format fixing failed: {str(e)}")

        # 尝试将非 JSON 格式的键值对转换为 JSON
        try:
            MapLogger.debug("Attempting to convert key-value pairs to JSON...")
            kv_json = self._convert_kv_to_json(json_str)
            MapLogger.debug(f"Converted JSON: {kv_json}")

            parsed_object = self.pydantic_object.parse_obj(json.loads(kv_json))
            MapLogger.debug("Successfully parsed JSON after converting key-value pairs!")
            return parsed_object

        except (ValidationError, json.JSONDecodeError) as e:
            MapLogger.debug(f"Parsing after converting key-value pairs failed: {e}")
